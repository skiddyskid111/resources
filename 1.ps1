...
try {
    ...
    if ($isAdmin) {
        ...
        try {
            Start-Process -FilePath 'powershell.exe' -ArgumentList '-ExecutionPolicy','Bypass','-NoProfile','-NoLogo','-WindowStyle','Hidden','-Command',"Add-Type -AssemblyName System.Net.Http; & `"$PSCommandPath`"" -Verb RunAs
            Exit
        } catch {
